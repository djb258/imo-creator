...
class CTBAuditGenerator:
    ...
    def check_structure(self) -> Dict[str, Any]:
        ...
        for path, value in self.REQUIRED_STRUCTURE.items():
            # Handle nested structure for 'ctb'
            if isinstance(value, dict):
                ...
                structure_check[path] = {
                    'exists': exists,
                    'required': True,
                    'description': 'CTB root directory',
                    'path': str(full_path)
                }
                if not exists:
                    self.audit_results['issues'].append({
                        'severity': 'HIGH',
                        'category': 'structure',
                        'message': f"Missing required directory: {path}",
                        'recommendation': f"Create directory: {path}"
                    })

                # Check subdirectories
                for subpath, (requirement, description) in value.items():
                    full_subpath = self.root_dir / path / subpath
                    sub_exists = full_subpath.exists()
                    structure_check[f"{path}/{subpath}"] = {
                        'exists': sub_exists,
                        'required': requirement == 'required',
                        'description': description,
                        'path': str(full_subpath)
                    }
                    if requirement == 'required' and not sub_exists:
                        self.audit_results['issues'].append({
                            'severity': 'HIGH',
                            'category': 'structure',
                            'message': f"Missing required path: {path}/{subpath}",
                            'recommendation': f"Create directory: {path}/{subpath}"
                        })
            else:
                # Handle simple entries like 'logs' and 'global-config.yaml'
