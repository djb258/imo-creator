...
class LovableAPI {
    apiKey;
    baseURL;
    constructor(apiKey, baseURL = LOVABLE_BASE_URL) {
        this.apiKey = apiKey;
    ...
    async getProjectStatus(projectId) {
        try {
            const response = await axios.get(`${this.baseURL}/projects/${projectId}/status`, {
                headers: {
                    'Authorization': `Bearer ${this.apiKey}`,
                },
            });
...
class ComposioFallback {
    ...
    async createProject(params) {
        try {
            ...
            const response = await axios.post('https://api.composio.dev/v1/actions/execute', {
...
function readCTBSpecs(specPath) {
