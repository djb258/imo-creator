...
**Five Validators:** Every candidate constant must survive all five simultaneously
(not sequentially). These are the Five Elements defined in OPERATOR_PROFILE.md:

1. **C&V:** Is this truly fixed, or is it a mislabeled variable? Does it name a
   specific tool or vendor? If so, it is a variable, not a constant.
2. **IMO:** Does it stay fixed regardless of what flows through the process?
3. **CTB:** Does it stay fixed at every level of the hierarchy?
4. **Hub-and-Spoke:** Does it stay fixed regardless of which physical interface,
   transport, or processing engine implements it? If swapping the implementation
   breaks it, it is a variable.
5. **Circle:** Does it still hold after a full feedback cycle?

**Block Architecture:** This skill is built from blocks. Each block is governed by one of
the five elements (C&V, IMO, CTB, Hub-and-Spoke, Circle). Every block — regardless of
its governing element — follows the same internal format defined by all five elements.
The number of blocks is determined by the domain. The format per block is the constant.

**Upstream Authority:** This skill conforms to OPERATOR_PROFILE.md. If any constant in
this skill contradicts a constant in OPERATOR_PROFILE.md, the OPERATOR_PROFILE prevails.
...
- Every candidate constant must survive all five validators simultaneously
  (C&V, IMO, CTB, Hub-and-Spoke, Circle).
...
- If swapping every vendor, tool, and format breaks the candidate, it is a variable
  (Hub-and-Spoke test).
...
- Middle: Run the gate mechanism. At each gate:
  ...
  2. Validate with C&V: Is this truly fixed, or a mislabeled variable? Does it name
  ...
  5. Validate with Hub-and-Spoke: Does it hold if you swap every implementation?
     If the candidate breaks when you change a vendor, tool, or format, it is a
...
- Leaves: The validation evidence for each constant (which of the five tests it passed).
...
evidence against all five elements. Proceed.
...
- If the skill has I/O boundaries (interfaces with external systems, users, or other
  skills), it must declare a Hub-and-Spoke Configuration table mapping each spoke to
  its input, output, and interface to the hub.
...
- Whether a Hub-and-Spoke Configuration table is required (determined by whether the
  skill has external I/O boundaries).
...
- Middle: Decompose the skill's Middle into execution phases. Each phase is a spoke
  ...
  If the skill interfaces with external systems: produce a Hub-and-Spoke Configuration
  table declaring each spoke (name, input, output, interface to hub, Go/No-Go).
  This is mandatory for agent skills and any skill with external I/O. It is optional
  for pure-logic skills with no external boundaries.
- Output: Complete workflow with phases, each structured as an IMO with Go/No-Go gates.
  Hub-and-Spoke Configuration table if applicable.

**CTB:**
- Trunk: The overall workflow.
- Branches: Individual phases + Hub-and-Spoke Configuration (if applicable).
- Leaves: Phase-specific constants, variables, IMO, gates, and spoke declarations.
...
- Validation: Walk through the workflow mentally with a concrete example. Does each
  ...
  pass/fail criterion? If Hub-and-Spoke table exists, does every spoke map cleanly
  to a workflow phase?
...
Concrete example walks through cleanly. Hub-and-Spoke table present if skill has
external I/O. Proceed.
...
- SKILL.md structure is always: Tier 0 Doctrine → IMO → Constants → Variables →
  Hub-and-Spoke Configuration (if applicable) → Workflow → Rules → Reference Pointers.
...
- Input: All outputs from Blocks 1-4 (scope, constants, variables, IMO, workflow,
  Hub-and-Spoke Configuration if applicable).
- Middle: Assemble the skill as a CTB:
  - Trunk: The SKILL.md file itself — the single source of truth.
  - Branches: Major sections (IMO, Constants, Variables, Hub-and-Spoke Configuration,
...
clean. Hub-and-Spoke Configuration present where required. Proceed.
...
- Middle: For each constant and each workflow phase, ask: "What is the most common way
  ...
  - Never validate constants against fewer than five elements.
  - Never omit Hub-and-Spoke Configuration when the skill has external I/O boundaries.
...
### BLOCK 7: Compliance Gate
**Governed by: Circle**

**Constants:**
- Every skill produced by the skill-creator must conform to OPERATOR_PROFILE.md.
- The compliance gate checks the assembled skill against the OPERATOR_PROFILE's
  constants before the swap test. If the skill violates any OPERATOR_PROFILE constant,
  it does not proceed to swap test — it returns to the block that produced the violation.
- The compliance gate uses the same three-tier enforcement model defined in
  OPERATOR_PROFILE.md Block 6:
  - Tier 1 (Self-Check): The generating LLM runs the gate against its own output.
  - Tier 2 (Cross-Check): A different inference engine runs the same gate. Which
    LLM fills this seat is a variable.
  - Tier 3 (Human/Circle): Dave spot-checks. Escapes harden the gate.
- All six OPERATOR_PROFILE gates fire against the skill output. A gate that does not
  apply auto-passes. The LLM does not decide which gates to skip.
- Maximum three rewrite attempts per gate failure. If still failing, escalate to the
  user: "Cannot produce this skill without violating an OPERATOR_PROFILE constant.
  Failing gate: [number and name]. Clarification needed."

**Variables:**
- The specific skill being audited.
- Which OPERATOR_PROFILE gates are most relevant to the skill's domain.
- Which LLM fills the Tier 2 cross-check seat.

**IMO:**
- Input: The assembled, rules-complete skill from Block 6.
- Middle: Run all six OPERATOR_PROFILE gates against the skill:
  - Gate 1 (Identity): Does the skill frame anything in feature/marketing language?
  - Gate 2 (Communication): Does the skill contain fluff, false enthusiasm, or
    performative hedging?
  - Gate 3 (Architecture): Does the skill name a vendor as a constant? Place logic
    in a spoke? Bleed pipeline roles?
  - Gate 4 (Decision): Does the skill force decisions without extracting constants?
  - Gate 5 (Vocabulary): Does the skill misuse a framework term?
  - Gate 6 (Cross-Block Integrity): Does the skill contradict any OPERATOR_PROFILE
    constant?
  Binary pass/fail per gate. Any FAIL = rewrite. Three attempts max, then escalate.
- Output: PASS (all gates cleared) or escalation identifying the structural conflict.

**CTB:**
- Trunk: Upstream compliance — the skill conforms to OPERATOR_PROFILE.
- Branches: Six gates + three-tier enforcement + rewrite protocol.
- Leaves: Specific gate results per skill.

**Circle:**
- Validation: Did a non-compliant skill pass through to swap test? If yes, this block
  failed. Trace which gate should have caught it.
- Feedback: Every escape hardens the gate. Repeated escapes of the same type indicate
  a principle gap — escalate to the user for a new constant.

**Go/No-Go:** All six OPERATOR_PROFILE gates passed. Skill is compliant with upstream
doctrine. Proceed to swap test.
...
**IMO:**
- Input: The compliance-cleared skill from Block 7.
...
- Feedback: FAIL routes correction to the specific block that produced the mismatch:
  ...
  - Upstream violation → return to Block 7 (compliance gate).

**Go/No-Go:** PASS = proceed to validation. FAIL = return to identified block.

---

### BLOCK 9: Automated Validation
**Governed by: Circle**
...
**IMO:**
- Input: Swap-tested skill from Block 8.
...
**IMO:**
- Input: Validated skill from Block 9.
...
- Feedback: Iteration begins — see Block 11.

**Go/No-Go:** Skill delivered. Move to iteration.

---

### BLOCK 11: Iteration Protocol
**Governed by: Circle**
...
- Middle: Diagnose the deviation:
  ...
  6. Is it an upstream compliance gap? → Return to Block 7, re-run gates.
  7. Is it unclassifiable (OTHER)? → Log to error table. Do not guess. Ask the user.
- Output: Specific correction routed to specific block. Re-run Blocks 7-10 after fix.

**CTB:**
- Trunk: The iteration cycle.
- Branches: Diagnosis categories (constants gap, rules gap, workflow gap, compliance
  gap, etc.).
...
unknowns. By the time the LLM reaches Block 10, the only things still open are the true
