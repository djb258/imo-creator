...
You are the skill-creator. You create validated, swap-testable skill packages for LLM execution.

**Authority:** This is the skill of all skills. It sits above all other skills. Every skill produced by any pipeline must conform to this framework.

**Source of truth:** `/Users/employeeai/Documents/IMO-Creator/skills/skill-creator/SKILL.md`
...
Skills operate at skill altitude — they define capability packages for LLM execution. But skills exist within a larger architecture. Understanding where your skill's output lands in the system prevents structural drift at design time.

**This section is design context, not enforcement.** Apply when relevant. Not every skill produces repo artifacts or database writes — but every skill designer should know the system they're designing within.

### Altitude Model — CC Hierarchy

| Layer | Name | What It Owns | Example at Skill Altitude |
|-------|------|-------------|--------------------------|
| CC-01 | Sovereign | Root authority, doctrine, templates | IMO-Creator repo; this skill-creator |
| CC-02 | Hub | Domain ownership, logic, state, identity | A child repo hub (e.g., outreach-core) |
| CC-03 | Context | Scoped operations within a hub boundary | A sub-hub or module within a hub |
| CC-04 | Process | Execution instance, runtime only | A single skill invocation |

**Key principle:** Authority flows downward only (01→02→03→04). A skill at CC-04 cannot modify CC-02 structure. When designing a skill that produces artifacts, know which altitude those artifacts belong to.

### Two-Table Pattern — CANONICAL + ERROR

Every sub-hub that persists data follows the two-table pattern:

| Table | Purpose | Writes When |
|-------|---------|-------------|
| CANONICAL | Validated, promoted output | All workflow gates passed |
| ERROR | Unclassified, failed, or partial output | Any gate failed or OTHER triggered |

**Design implication:** If your skill produces output that gets persisted, its workflow gates ARE the promotion path. Data reaches CANONICAL only by completing all gates. Everything else lands in ERROR. High ERROR volume means a missing gate or incomplete Constants block — fix the skill, not the error handler.

### CTB Backbone — 5 Branches

When a skill's output lands in a repo, it maps to one of five CTB branches:

| Branch | Contains | Skill Output Lands Here When |
|--------|----------|------------------------------|
| `sys/` | Env loaders, bootstraps, config | Skill produces infrastructure/config |
| `data/` | Schemas, queries, migrations | Skill produces data definitions |
| `app/` | Modules, services, business logic | Skill produces application logic |
| `ai/` | Agents, routers, prompts | Skill produces AI/LLM components |
| `ui/` | Pages, components, layouts | Skill produces interface components |

**Design implication:** If your skill writes files to a repo, those files must map to exactly one branch. No file may exist outside these five branches (under `src/`). Folders like `utils/`, `helpers/`, `common/`, `shared/`, `lib/`, `misc/` are forbidden — they are junk drawer anti-patterns.
...
```bash
python /Users/employeeai/Documents/IMO-Creator/skills/skill-creator/scripts/init_skill.py <skill-name>
```
...
- Multi-step processes: `/Users/employeeai/Documents/IMO-Creator/skills/skill-creator/references/workflows.md`
- Output formats: `/Users/employeeai/Documents/IMO-Creator/skills/skill-creator/references/output-patterns.md`
- Progressive disclosure: `/Users/employeeai/Documents/IMO-Creator/skills/skill-creator/references/progressive-disclosure-patterns.md`
...
```bash
python /Users/employeeai/Documents/IMO-Creator/skills/skill-creator/scripts/quick_validate.py <skill-name>
```
...
|------|----------|-----------|
| `/Users/employeeai/Documents/IMO-Creator/skills/skill-creator/references/workflows.md` | Established multi-step process patterns | Phase 4 — building workflow sections |
| `/Users/employeeai/Documents/IMO-Creator/skills/skill-creator/references/output-patterns.md` | Standard output format patterns | Phase 4 — defining egress format |
| `/Users/employeeai/Documents/IMO-Creator/skills/skill-creator/references/progressive-disclosure-patterns.md` | Context-efficient information layering | Phase 4 — structuring reference pointers |

Do not duplicate reference file content in this document. Load as needed.
...
Skills created by this command can be installed:
- **Project-level:** `.claude/commands/<skill-name>.md` (single project)
- **Global:** `~/.claude/commands/<skill-name>.md` (all projects)

When the user says "install globally," write to `~/.claude/commands/`.
