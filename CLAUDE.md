...
| `docs/CTB_DRIFT_BASELINE.json` | Captures known legacy drift state (child repo output — generated by `ctb-drift-audit.sh --write-baseline`) |
...
| Bootstrap attestation | `bootstrap-audit.sh` → `docs/BOOTSTRAP_AUDIT.md` present with PASS (child repo output) |
